 ...
 ***********************************************************************************/
 u8 M8266WIFI_Module_Init_Via_SPI(void)
 {
	...
	char 	sta_ip[15+1]={0};
	char 	ssid[32];
//	u8 	ssid1[32]="HUAWEI-L6VJ9E";
//	u8  password[32]= "hzx121766";
//	u8 	ssid1[32]="HUAWEI-KZ2YYH";
//	u8  password[32]= "hzx121766";	
	u8 	ssid1[32]="hzc_design";
	u8  password[32]= "12345678";		
...
#define SPI_BaudRatePrescaler_256       ((uint32_t)0x00000008U)
	...
	#endif
	//2.20计划修改分频时间，降低spi频率由21MHZ降为10.5MHZ  3.9改回来试试。。 照片太慢
	 spi_clk = 21000000;                                                                       
...
//u8 M8266WIFI_SPI_Get_Opmode(u8* op_mode, u16* status);	
	   ...
	   ||(sta_ap_mode == 3))  // if STA+AP mode
	{
		...
		//u8 M8266WIFI_SPI_STA_Connect_Ap(u8 ssid[32], u8 password[64], u8 saved, u8 timeout_in_s, u16* status);
				 //if(M8266WIFI_SPI_STA_Connect_Ap("HUAWEI-L6VJ9E", "hzx121766", 0, 20, &status) == 0)
				 
				 
 		     if(M8266WIFI_SPI_STA_Connect_Ap(ssid1, password, 0, 20, &status) == 0) // not saved, timeout=20s 之前的参数和上面config一样
				 return 0;			 			 //现在修改为家里WiFi参数
#endif
